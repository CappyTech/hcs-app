...
exports.renderCreateAssignmentForm = async (req, res, next) => {
  try {
    const contract = await mdb.contract.findById(req.params.contractId);
    if (!contract) {
      req.flash('error', 'Contract not found.');
      return res.redirect('/contracts');
    }
...
exports.renderUpdateAssignmentForm = async (req, res, next) => {
  try {
    const assignment = await mdb.ContractAssignment.findById(req.params.id)
      .populate('contractId')
      .populate('assignedEmployees')
