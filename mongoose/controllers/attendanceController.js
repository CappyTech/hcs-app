...
exports.getWeeklyAttendance = async (req, res, next) => {
  try {
    ...
    const stripPay = req.user && !['admin', 'accountant'].includes(req.user.role);

    const employeeEntries = Object.entries(scopedGrouped)
      .filter(([_, v]) => v.type === 'employee')
      .map(([uuid, v]) => [uuid, (isManagementView || stripPay) ? stripPayroll(v) : v]);
...
exports.renderSubmitAttendance = async (req, res, next) => {
  try {
    ...
    const [projects, locations] = await Promise.all([
      mdb.REST?.project?.find({ $or: [{ Status: 0 }, { Status: 2 }] }).select('uuid Name Number Status').lean() || [],
    ...
    if (isEmployee && req.user.employeeId) {
      const emp = await mdb.INTERNAL.employee.findById(req.user.employeeId).select('name').lean();
      if (emp) entityName = emp.name;
    } else if (!isEmployee && req.user.subcontractorId) {
      const sub = await mdb.REST.supplier.findById(req.user.subcontractorId).select('Name').lean();
  ...
  try {
    ...
    if (isEmployee) {
      ...
      if (!req.user.subcontractorId) {
        req.flash('error', 'Your account is not linked to a subcontractor record.');
        return res.redirect('/attendance/submit');
      }
    ...
    logger.info(`📝 Self-service attendance submitted by ${req.user.username} (${role}): ${record.uuid}`);
    ...
    if (err.code === 11000) {
      ...
      req.flash('error', 'You already have an attendance record for this date/location/project combination.');
      return res.redirect('/attendance/submit');
    }
