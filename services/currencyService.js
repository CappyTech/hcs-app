  ...
  try {
    ...
    const expenseData = await db.KF_Receipts.findAll({
      ...
      attributes: [
        [db.Sequelize.fn("DATE", db.Sequelize.col("InvoiceDate")), "date"],
        [
          db.Sequelize.fn("SUM", db.Sequelize.col("AmountPaid")),
          "totalExpenses",
        ],
      ],
      group: ["date"],
    ...
    dateRange.reverse().forEach((date) => {
      const income =
        incomeData.find((entry) => entry.date === date)?.totalIncome || 0;
      const expenses =
        expenseData.find((entry) => entry.date === date)?.totalExpenses || 0;
